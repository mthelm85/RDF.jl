# SPARQL 1.1 Query Results serialization
#
# Implements the three W3C standard wire formats for SPARQL results:
...
#
# Each format supports both SolutionSet (SELECT results) and Bool (ASK results).
# All four formats are write-only in this release; reading SPARQL result documents
# is straightforward to add later (useful for consuming remote SPARQL endpoints).
